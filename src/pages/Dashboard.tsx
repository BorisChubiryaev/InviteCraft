...
import {
  Plus, Edit3, Trash2, Globe, GlobeLock, ExternalLink, Users,
  Calendar, Copy, CheckCircle2, BarChart3
} from 'lucide-react';
...
export default function Dashboard() {
  const { user } = useAuth();
  const { getUserSites, deleteSite, publishSite, unpublishSite, getRSVPResponses } = useSites();
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-gray-500 mt-1">Управляйте вашими приглашениями</p>
          </div>
          <Link
            to="/new"
        ...
        {userSites.length === 0 ? (
          <div className="bg-white rounded-3xl border border-gray-100 p-16 text-center">
            ...
            <h3 className="text-xl font-bold text-gray-900 mb-2">У вас пока нет сайтов</h3>
            <p className="text-gray-500 mb-6">Создайте своё первое приглашение прямо сейчас!</p>
            <Link
              to="/new"
              ...
              return (
                ...
                >
                  <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                    {/* Preview */}
                    <div className={`w-full lg:w-24 h-20 lg:h-16 rounded-xl bg-gradient-to-br ${tpl?.previewGradient || 'from-gray-100 to-gray-200'} flex items-center justify-center flex-shrink-0`}>
                      <span className="text-3xl">{tpl?.previewEmoji || '📄'}</span>
                    </div>

                    {/* Info */}
                    <div className="flex-grow min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <h3 className="font-bold text-gray-900 text-lg truncate">{site.config.names || site.config.title}</h3>
                        {site.published ? (
                    ...
                    {rsvpCount > 0 && (
                      <button
                        onClick={() => setShowResponses(showResponses === site.id ? null : site.id)}
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      {site.published ? (
                        <>
                          ...
                          >
                            {copiedId === site.id ? <CheckCircle2 className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
                          </button>
                          <Link
                            to={`/p/${site.id}`}
                            target="_blank"
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-100 animate-slide-down">
                          ...
                          <tbody>
                            {responses.map(r => (
                              <tr key={r.id} className="border-b border-gray-50">
                                <td className="py-2 pr-4 font-medium text-gray-700">{r.name}</td>
                                <td className="py-2 pr-4 text-gray-500">{r.phone || '—'}</td>
                                <td className="py-2 pr-4">
                                  <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${
                                    r.attendance === 'yes' ? 'bg-green-50 text-green-600' :
                                    r.attendance === 'no' ? 'bg-red-50 text-red-500' :
                                    'bg-yellow-50 text-yellow-600'
                                  }`}>
                                    {r.attendance === 'yes' ? 'Придёт' : r.attendance === 'no' ? 'Не придёт' : 'Может быть'}
                                  </span>
                                </td>
                                <td className="py-2 pr-4 text-gray-500">{r.guestsCount}</td>
                                <td className="py-2 text-gray-500 truncate max-w-xs">{r.message || '—'}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                      ...
                      <div className="mt-3 flex gap-4 text-sm">
                        <span className="text-green-600">
                          ✓ Придут: {responses.filter(r => r.attendance === 'yes').length}
                        </span>
                        <span className="text-red-500">
                          ✗ Не придут: {responses.filter(r => r.attendance === 'no').length}
                        </span>
                        <span className="text-yellow-600">
                          ? Может быть: {responses.filter(r => r.attendance === 'maybe').length}
                        </span>
                        <span className="text-gray-500">
                          Всего гостей: {responses.filter(r => r.attendance !== 'no').reduce((sum, r) => sum + r.guestsCount, 0)}
                        </span>
                      </div>
                    </div>
                  )}
