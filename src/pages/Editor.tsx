...
import {
  ArrowLeft, Save, Globe, GlobeLock, Eye, EyeOff,
  Plus, Trash2, CheckCircle2, ChevronDown, ChevronRight,
  X, Monitor, Smartphone, Copy, ExternalLink, QrCode, GripVertical,
  Image, Clock, MapPin, Calendar, Users, Film, Heart, Shirt, Palette, Type
} from 'lucide-react';
...
const COLOR_PRESETS = [
  { name: 'Золото', primary: '#B8860B', accent: '#D4AF37', secondary: '#F5F0E8', bg: '#FFFEF7', text: '#2C2C2C' },
  { name: 'Розовый', primary: '#D4798A', accent: '#E8A0B0', secondary: '#FDF2F4', bg: '#FFF5F7', text: '#4A2C3D' },
  { name: 'Синий', primary: '#2563EB', accent: '#3B82F6', secondary: '#EFF6FF', bg: '#FFFFFF', text: '#1E293B' },
  { name: 'Зелёный', primary: '#059669', accent: '#10B981', secondary: '#ECFDF5', bg: '#FFFFFF', text: '#064E3B' },
  { name: 'Фиолетовый', primary: '#7C3AED', accent: '#A78BFA', secondary: '#F5F3FF', bg: '#FEFCFF', text: '#1E1B4B' },
  { name: 'Минимализм', primary: '#1A1A1A', accent: '#C9A96E', secondary: '#F5F5F5', bg: '#FFFFFF', text: '#1A1A1A' },
  { name: 'Терракота', primary: '#C2410C', accent: '#EA580C', secondary: '#FFF7ED', bg: '#FFFBF5', text: '#431407' },
  { name: 'Лаванда', primary: '#7E22CE', accent: '#A855F7', secondary: '#FAF5FF', bg: '#FDFCFE', text: '#3B0764' },
  { name: 'Оливковый', primary: '#4D7C0F', accent: '#84CC16', secondary: '#F7FEE7', bg: '#FEFFF5', text: '#1A2E05' },
  { name: 'Бордовый', primary: '#9F1239', accent: '#BE123C', secondary: '#FFF1F2', bg: '#FFFBFC', text: '#4C0519' },
];
      ...
      <div className="flex items-center gap-2 p-3">
        ...
        <button
          ...
          className={cn(
            ...
            section.enabled ? "bg-indigo-100 text-indigo-600" : "bg-gray-100 text-gray-400"
          )}
        >
...
export default function Editor() {
  ...
  useEffect(() => {
    ...
    if (!site) { navigate('/dashboard'); return; }
    if (site.userId !== user.id) { navigate('/dashboard'); return; }
    if (!localConfig) {
      setLocalConfig({ 
        ...site.config, 
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      const oldIndex = localConfig.sections.findIndex((s) => s.id === active.id);
      const newIndex = localConfig.sections.findIndex((s) => s.id === over.id);
      const newSections = arrayMove(localConfig.sections, oldIndex, newIndex).map((s, idx) => ({
        ...s,
        order: idx
      }));
  ...
  const updateScheduleItem = (idx: number, updates: Partial<ScheduleItem>) => {
    const newSchedule = localConfig.schedule.map((item, i) => i === idx ? { ...item, ...updates } : item);
    updateConfig({ schedule: newSchedule });
  };

  const removeScheduleItem = (idx: number) => {
    updateConfig({ schedule: localConfig.schedule.filter((_, i) => i !== idx) });
  };
  ...
  const removeGalleryImage = (idx: number) => {
    updateConfig({ galleryImages: localConfig.galleryImages.filter((_, i) => i !== idx) });
  };
  ...
  const updateVideo = (idx: number, updates: Partial<VideoItem>) => {
    const videos = (localConfig.videos || []).map((v, i) => i === idx ? { ...v, ...updates } : v);
    updateConfig({ videos });
  };

  const removeVideo = (idx: number) => {
    updateConfig({ videos: (localConfig.videos || []).filter((_, i) => i !== idx) });
  };
  ...
  const renderSectionSettings = (section: SectionConfig) => {
    switch (section.type) {
      case 'hero':
        return (
          <div className="space-y-4">
            <InputField label="Имена / Название" value={localConfig.names} onChange={v => updateConfig({ names: v })} placeholder="Александр & Екатерина" />
            <InputField label="Заголовок" value={localConfig.title} onChange={v => updateConfig({ title: v })} placeholder="Мы женимся!" />
            <InputField label="Подзаголовок" value={localConfig.subtitle} onChange={v => updateConfig({ subtitle: v })} placeholder="Приглашаем вас..." />
            <div className="grid grid-cols-2 gap-3">
              <InputField label="Дата" type="date" value={localConfig.date} onChange={v => updateConfig({ date: v })} />
              <InputField label="Время" type="time" value={localConfig.time} onChange={v => updateConfig({ time: v })} />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Обложка (фото/GIF)</label>
              <ImageUpload 
            ...
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Затемнение: {localConfig.heroOverlay || 40}%</label>
              <input
                type="range"
                min={0}
                max={80}
                value={localConfig.heroOverlay || 40}
                onChange={e => updateConfig({ heroOverlay: Number(e.target.value) })}
                className="w-full accent-indigo-500"
              />
            </div>
            <ToggleField label="Градиентный текст" value={localConfig.gradientText || false} onChange={v => updateConfig({ gradientText: v })} />
            <ToggleField label="Параллакс эффект" value={localConfig.parallaxHero || false} onChange={v => updateConfig({ parallaxHero: v })} />
          </div>
        );
      ...
      case 'countdown':
        return (
          <div className="space-y-4">
            <ToggleField label="Показывать обратный отсчёт" value={localConfig.countdownEnabled} onChange={v => updateConfig({ countdownEnabled: v })} />
            <div className="grid grid-cols-2 gap-2">
              {(['classic', 'minimal', 'cards', 'circles'] as const).map(style => (
                <button
                  key={style}
      ...
      case 'details':
        return (
          <div className="space-y-4">
            <InputField label="Заголовок раздела" value={localConfig.detailsTitle || localConfig.title} onChange={v => updateConfig({ detailsTitle: v })} />
            <TextAreaField label="Описание" value={localConfig.description} onChange={v => updateConfig({ description: v })} rows={3} />
            <InputField label="Место проведения" value={localConfig.venue} onChange={v => updateConfig({ venue: v })} placeholder="Название площадки" />
            <InputField label="Адрес" value={localConfig.address} onChange={v => updateConfig({ address: v })} placeholder="Полный адрес" />
          </div>
        );
      ...
      case 'schedule':
        return (
          <div className="space-y-4">
            <InputField label="Заголовок раздела" value={localConfig.scheduleTitle || 'Программа'} onChange={v => updateConfig({ scheduleTitle: v })} />
              ...
              {localConfig.schedule.map((item, idx) => (
                <div key={item.id || idx} className="p-3 bg-white rounded-lg border border-gray-200 space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <button onClick={() => removeScheduleItem(idx)} className="text-red-400 hover:text-red-600 p-1">
      ...
      case 'gallery':
        return (
          <div className="space-y-4">
            <InputField label="Заголовок раздела" value={localConfig.galleryTitle || 'Галерея'} onChange={v => updateConfig({ galleryTitle: v })} />
            <div className="grid grid-cols-2 gap-3">
              <SelectField 
                label="Раскладка" 
              ...
              <SelectField 
                ...
                onChange={v => updateConfig({ galleryColumns: Number(v) as any })}
                options={[
                  { value: '2', label: '2' },
                  { value: '3', label: '3' },
                  { value: '4', label: '4' },
            ...
            <div className="space-y-3">
              {localConfig.galleryImages.map((img, idx) => (
                <div key={idx} className="bg-white p-2 rounded-lg border border-gray-200 relative">
                  <button 
      ...
      case 'video':
        return (
          <div className="space-y-4">
            <InputField label="Заголовок раздела" value={localConfig.videosTitle || 'Видео'} onChange={v => updateConfig({ videosTitle: v })} />
            <p className="text-xs text-gray-500">Поддерживаются ссылки: YouTube, VK Video, RuTube</p>
            <div className="space-y-3">
              {(localConfig.videos || []).map((video, idx) => (
                <div key={video.id} className="p-3 bg-white rounded-lg border border-gray-200 space-y-2">
                  <div className="flex gap-2">
                    ...
                    />
                    <button onClick={() => removeVideo(idx)} className="text-red-400 hover:text-red-600 p-1">
                      <Trash2 className="w-4 h-4" />
                    </button>
                  </div>
                  <input
      ...
      case 'map':
        return (
          <div className="space-y-4">
            <InputField label="Заголовок раздела" value={localConfig.mapTitle || 'Как добраться'} onChange={v => updateConfig({ mapTitle: v })} />
            <ToggleField label="Показывать карту" value={localConfig.mapEnabled} onChange={v => updateConfig({ mapEnabled: v })} />
            <InputField label="Место" value={localConfig.venue} onChange={v => updateConfig({ venue: v })} placeholder="Название" />
            <InputField label="Адрес для карты" value={localConfig.address} onChange={v => updateConfig({ address: v })} placeholder="Полный адрес" />
          </div>
        );
      ...
      case 'dresscode':
        return (
          <div className="space-y-4">
            <TextAreaField label="Описание дресс-кода" value={localConfig.dressCode} onChange={v => updateConfig({ dressCode: v })} rows={3} />
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Цвета дресс-кода</label>
              <div className="flex gap-2 flex-wrap">
                {(localConfig.dressCodeColors || []).map((color, idx) => (
                  <div key={idx} className="relative group">
                    ...
                    <button 
                      onClick={() => {
                        const colors = (localConfig.dressCodeColors || []).filter((_, i) => i !== idx);
                        updateConfig({ dressCodeColors: colors });
                      }}
                ...
                <button 
                  onClick={() => updateConfig({ dressCodeColors: [...(localConfig.dressCodeColors || []), '#CCCCCC'] })}
      ...
      case 'rsvp':
        return (
          <div className="space-y-4">
            <ToggleField label="Включить форму RSVP" value={localConfig.rsvpEnabled} onChange={v => updateConfig({ rsvpEnabled: v })} />
            <InputField label="Заголовок" value={localConfig.rsvpTitle || 'Подтвердите участие'} onChange={v => updateConfig({ rsvpTitle: v })} />
            <TextAreaField label="Описание" value={localConfig.rsvpDescription || ''} onChange={v => updateConfig({ rsvpDescription: v })} rows={2} />
          </div>
        );
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-100">
        ...
        <div className="flex items-center gap-2">
          <div className="hidden lg:flex items-center gap-1 bg-gray-100 rounded-lg p-0.5">
            <button
              ...
              className={cn("p-1.5 rounded-md transition-colors", previewMode === 'desktop' ? 'bg-white shadow text-indigo-600' : 'text-gray-400')}
            >
              <Monitor className="w-4 h-4" />
            </button>
            <button
              onClick={() => setPreviewMode('mobile')}
              className={cn("p-1.5 rounded-md transition-colors", previewMode === 'mobile' ? 'bg-white shadow text-indigo-600' : 'text-gray-400')}
            >
          ...
          <button
            ...
            className="lg:hidden flex items-center gap-1.5 px-3 py-1.5 text-sm text-gray-600 hover:text-gray-800 bg-gray-100 rounded-lg"
          >
          ...
          >
            {site.published ? <GlobeLock className="w-4 h-4" /> : <Globe className="w-4 h-4" />}
            <span className="hidden sm:inline">{site.published ? 'Снять' : 'Опубликовать'}</span>
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className={cn(
          ...
          <button
            ...
            className={cn(
              ...
              showGlobalSettings ? "bg-indigo-50 border-indigo-100" : "bg-white border-gray-200 hover:bg-gray-50"
            )}
          >
            <div className="flex items-center gap-3">
              <div className={cn(
                "w-8 h-8 rounded-lg flex items-center justify-center",
                showGlobalSettings ? "bg-indigo-500 text-white" : "bg-gray-100 text-gray-500"
              )}>
                <Palette className="w-4 h-4" />
              </div>
              <span className="font-medium text-gray-800">Общие настройки</span>
            </div>
            {showGlobalSettings ? <ChevronDown className="w-4 h-4 text-gray-400" /> : <ChevronRight className="w-4 h-4 text-gray-400" />}
          </button>
          ...
          {showGlobalSettings && (
            <div className="p-4 bg-white border-b border-gray-200 space-y-4 max-h-[50vh] overflow-y-auto">
                  ...
                  {COLOR_PRESETS.map(preset => (
                    ...
                    >
                      <div className="flex -space-x-1 justify-center p-2 border border-gray-200 rounded-lg hover:border-indigo-300 transition-colors">
                        ...
                        <div className="w-4 h-4 rounded-full border border-white shadow-sm" style={{ backgroundColor: preset.accent }} />
                      </div>
                    </button>
                  ))}
                </div>
              ...
              {/* Custom Colors */}
              <div className="grid grid-cols-2 gap-3">
                <ColorField label="Основной" value={localConfig.colorPrimary} onChange={v => updateConfig({ colorPrimary: v })} />
                <ColorField label="Акцент" value={localConfig.colorAccent} onChange={v => updateConfig({ colorAccent: v })} />
                <ColorField label="Фон" value={localConfig.colorBackground} onChange={v => updateConfig({ colorBackground: v })} />
                <ColorField label="Текст" value={localConfig.colorText} onChange={v => updateConfig({ colorText: v })} />
              </div>
              ...
              {/* Dividers */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Разделители</label>
                <div className="flex gap-2">
                  {(['none', 'line', 'dots', 'hearts', 'flowers'] as const).map(style => (
                    <button
                      key={style}
              ...
              {/* Effects */}
              <ToggleField label="Плавающие элементы" value={localConfig.floatingElements || false} onChange={v => updateConfig({ floatingElements: v })} />
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={e => updateConfig({ socialLinks: { ...localConfig.socialLinks, instagram: e.target.value } })}
                ...
                <input
                  ...
                  onChange={e => updateConfig({ socialLinks: { ...localConfig.socialLinks, telegram: e.target.value } })}
                  placeholder="Telegram @username"
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-indigo-200"
                />
                <input
                  value={localConfig.socialLinks?.hashtag || ''}
                  onChange={e => updateConfig({ socialLinks: { ...localConfig.socialLinks, hashtag: e.target.value } })}
                  placeholder="Хештег #НашаСвадьба"
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-indigo-200"
                />
                <ToggleField label="Показать в футере" value={localConfig.showSocialInFooter || false} onChange={v => updateConfig({ showSocialInFooter: v })} />
              </div>

              {/* Footer */}
              <TextAreaField label="Текст в футере" value={localConfig.footerText || ''} onChange={v => updateConfig({ footerText: v })} rows={2} />
            </div>
          )}
        ...
        {/* Preview */}
        <div className={cn("flex-1 bg-gray-100 overflow-hidden relative", !showPreview ? 'hidden lg:block' : '')}>
          <div className="h-full flex items-start justify-center overflow-auto p-4 lg:p-8">
            ...
            >
              ...
              <div className={cn("overflow-y-auto h-full", previewMode === 'mobile' ? 'pt-6' : '')} style={{ maxHeight: 'calc(100vh - 120px)' }}>
                <SiteRenderer 
                  config={localConfig} 
                  siteId={site.id} 
                  isPreview={true}
      ...
      {showQR && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm relative shadow-xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <h3 className="text-lg font-bold text-center mb-6">QR код приглашения</h3>
...
function ColorField({ label, value, onChange }: {
  ...
  const presetColors = [
    '#B8860B', '#D4AF37', '#FFD700', '#FFA500', '#FF6347',
    '#D4798A', '#E8A0B0', '#FF69B4', '#FF1493', '#DC143C',
    '#9F1239', '#BE123C', '#C2410C', '#EA580C', '#F97316',
    '#059669', '#10B981', '#22C55E', '#84CC16', '#4D7C0F',
    '#2563EB', '#3B82F6', '#0EA5E9', '#06B6D4', '#14B8A6',
    '#7C3AED', '#8B5CF6', '#A855F7', '#D946EF', '#EC4899',
    '#1A1A1A', '#374151', '#6B7280', '#9CA3AF', '#D1D5DB',
    '#FFFFFF', '#F5F5F5', '#FAFAFA', '#FEF3C7', '#FFFEF7',
  ];
